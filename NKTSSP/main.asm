...
.client_recv:
  ...
  je      .client_recv_headers
  cmp     [client.state], CLIENT_RECV_BODY
  je      .client_recv_body_data
  err_raw "invalid client recv state"

.client_recv_headers:
  ; Existing header reception logic
...
@@:
  ...
  jnz     .switch_end ; no match yet

  call    parse_request
  
  ; Check if we need to wait for body data
  cmp     [client.state], CLIENT_RECV_BODY
  je      .switch_end  ; Wait for body
  
  ; Switch to output mode for methods without body or after body processing
  mov     [ev_event], EPOLLOUT
  mov     [ev_data], r14
  Syscall SYS_epoll_ctl, [epollfd], EPOLL_CTL_MOD, [client.fd], ev
  jmp     .switch_end

.client_recv_body_data:
  ; Receive POST/PUT body data
  mov     rax, [client.content_len]
  sub     rax, [client.body_received]
  cmp     rax, 0
  je      .body_complete
  
  ; Limit read to remaining buffer space or remaining content
  cmp     rax, 1024
  jle     @f
  mov     rax, 1024
@@:
  
  ; Calculate buffer position for body data
  lea     r11, [client.request]
  add     r11, [client.request_len]  ; Skip headers
  mov     rbx, [client.body_received]
  add     r11, rbx
  
  Syscall SYS_recvfrom, [client.fd], r11, rax, 0, NULL, NULL
  cmp     rax, 0
  jg      @f
...
@@:
  add     rax, [client.body_received]
  mov     [client.body_received], rax
  
  ; Check if we've received all body data
  cmp     rax, [client.content_len]
  jl      .switch_end

.body_complete:
  ; Process the received body based on method
  mov     eax, [client.method]
  cmp     eax, HTTP_POST
  je      .process_post_body
  cmp     eax, HTTP_PUT
  je      .process_put_body
  jmp     .client_close  ; Shouldn't happen

.process_post_body:
  call    create_resource_with_body
  jmp     .send_response

.process_put_body:
  call    update_resource_with_body
  jmp     .send_response
...
.client_send:
  ...
  je      .client_send_headers
  cmp     [client.state], CLIENT_SEND_JSON    
  je      .client_send_json                   
...
.client_send_json:
  ; Send JSON headers first
  lea     rsi, [client.header]
  mov     eax, [client.header_pos]
  add     rsi, rax
  mov     edx, [client.header_len]
  sub     edx, eax
  xor     r10, r10
  xor     r8,  r8
  xor     r9,  r9
...
@@:
  ...
  ; Now send JSON body
  lea     rsi, [client.json_data]
  mov     edx, [client.json_len]
  xor     r10, r10
  xor     r8,  r8
  xor     r9,  r9
  Syscall SYS_sendto, [client.fd]
...
@@:
  ; JSON sent successfully, close connection
  jmp     .client_close
